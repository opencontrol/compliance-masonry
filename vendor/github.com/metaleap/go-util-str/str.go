...
func AnyOf (val string, anyof ...string) bool {
	for _,v := range anyof { if v==val { return true } }
	return false
}
func After (val string, needle string, elseempty bool) string {
	if i := strings.Index(val, needle) ; i>=0 { return val[i+1:] }
	if elseempty { return "" }
	return val
}
func AfterLast (val string, needle string, elseempty bool) string {
	if i := strings.LastIndex(val, needle) ; i>=0 { return val[i+1:] }
	if elseempty { return "" }
	return val
}
func Before (val string, needle string, elseempty bool) string {
	if i := strings.Index(val, needle) ; i>=0 { return val[:i] }
	if elseempty { return "" }
	return val
}
...
func DistBetween (val string, needle1 string, needle2 string) int {
	if i1 := strings.Index(val, needle1) ; i1>=0 {
		if len(needle2)==0 { return len(val) - (i1 + len(needle1)) }
		if i2 := strings.Index(val[i1+len(needle1):], needle2) ; i2>=0 { return i2 }
	}
	return -1
}
...
func BreakAt(val string, index int) (left string, right string) {
	left = val[:index]  ;  right = val[index:]  ;  return
}
